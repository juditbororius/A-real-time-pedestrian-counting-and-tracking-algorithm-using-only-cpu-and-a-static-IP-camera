...
def inside(r, q): #prove if the rectangle is inside another rectangle
    if var.model == 'MOG2':
    ...
    if var.model == 'HOG':
        rx, ry, rw, rh = r
        qx, qy, qw, qh = q
    return ((rx > qx and ry > qy and rx + rw < qx + qw and ry + rh < qy + qh) or (rx < qx and ry < qy and rx + rw > qx + qw and ry + rh > qy + qh))
...
def contour_calculations(contours, ident, medianFrame, realFrame, dataframe, Icount, memory):
    ...
    for ci, c in enumerate(contours):
        ...
        #print('Bounding box {} of frame {} has top left point in {} and bottom right {}'.format(count_people, Icount, bb.topleft, bb.bottomright))

    return ident, curr, centers, dataframe, count_people, fg_bounding

def id_connections(bb, memory, ident, min_tmp):
    #el minim s'ha d'escalar manualment al instal·lar la càmara
    minident = -1
    prevcenter = 0
    theorylist = []
...
def creation_in_out_spaces(shop):
    ...
    if shop.its_h:
        shop_in_space = [[shop.start.x - shop.extense, shop.start.y],
                         [shop.end.x, shop.end.y],
                         [shop.end.x, shop.end.y],
                         [shop.start.x - shop.extense, shop.start.y]]

        shop_out_space = [[shop.start.x, shop.start.y],
                       [shop.end.x + shop.extense, shop.end.y],
                       [shop.end.x + shop.extense, shop.end.y],
                       [shop.start.x, shop.start.y + shop.extense]]

    #esta hecho para que la tienda mire hacia abajo y no hacia arriba
    else:
        shop_in_space = [[shop.start.x, shop.start.y - shop.extense],
                         [shop.end.x, shop.end.y - shop.extense],
                         [shop.end.x, shop.end.y],
                         [shop.start.x, shop.start.y]]

        shop_out_space = [[shop.start.x - shop.espace, shop.start.y - shop.espace],
                       [shop.end.x + shop.espace, shop.end.y - shop.espace],
                       [shop.end.x + shop.espace, shop.end.y + shop.extense - shop.espace],
                       [shop.start.x - shop.espace, shop.start.y + shop.extense - shop.espace]]
        ...
        if curr_bb.counted == False:
            ...
            if abs(direction) > 15:
                if direction > 0 and isinside(shop_out_space, (curr_bb.center.x, curr_bb.center.y)):  # negative direction
                    ...
                    #print(direction, curr_bb.ident, 'DOWN SPACE', 'NOW IS COUNTED!!!')
                if direction < 0 and isinside(shop_in_space, (curr_bb.center.x, curr_bb.center.y)):  # positive direction
                    var.total_in += 1
                    curr_bb.counted = True
                    #print(direction, curr_bb.ident, 'UP SPACE', 'NOW IS COUNTED!!!')
        else:
            curr_bb.counted = True
    return curr_bb
